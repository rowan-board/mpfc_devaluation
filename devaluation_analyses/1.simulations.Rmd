...
lr = parameters[7:28,2]
inv_m = parameters[95:116,2]
inv_tau = parameters[73:94,2]
...
Qinits = matrix(0, nrow=Nrwd,ncol=Nopt)
Vinits = rep(0.0,2)
Minits = rep(1,2)
...
chosenNet <- as.data.frame(chosenNet) %>%
  pivot_longer(everything()) %>%
  rename(chosenNet = value)

chosenDevalued <- as.data.frame(chosenDevalued) %>%
  pivot_longer(everything()) %>%
  rename(chosenDevalued = value)

rwdDevalued <- as.data.frame(rwd_devalued) %>%
  pivot_longer(everything()) %>%
  rename(rwdDevalued = value)

winCurr <- as.data.frame(winCurr) %>%
  pivot_longer(everything()) %>%
  rename(winCurrency = value)

chosenRed <- as.data.frame(chosenRed) %>%
  pivot_longer(everything()) %>%
  rename(chosenRed = value)

chosenGreen <- as.data.frame(chosenGreen) %>%
  pivot_longer(everything()) %>%
  rename(chosenGreen = value)

stateTimeT <- as.data.frame(stateTimeT) %>%
  pivot_longer(everything()) %>%
  rename(stateTime = value)

isPurseFull_1 <- as.data.frame(isPurseFull_1) %>%
  pivot_longer(everything()) %>%
  rename(isPurseFull_1 = value)

isPurseFull_2 <- as.data.frame(isPurseFull_2) %>%
  pivot_longer(everything()) %>%
  rename(isPurseFull_2 = value)

data <- cbind(data, rwd, stateT, MR, MG, VA, VB, chosenNet, chosenDevalued, rwdDevalued, winCurr, chosenRed, chosenGreen, stateTimeT, isPurseFull_1, isPurseFull_2)

data <- data %>%
  select(chosenDice,totalWin,state,stateTime,MR,MG,isPurseFull_1,isPurseFull_2,VA,VB,chosenNet,chosenDevalued,rwdDevalued, winCurrency, chosenRed, chosenGreen) %>%
  mutate(vDiff = chosenNet - chosenDevalued)
data['subject_nr'] <- rep(1:22, each = (nBlocks*NTr))
data['allTrialIndex'] <- rep(1:(nBlocks*NTr), 22)

mapping = c('learning', 'learning', 'fill', 'extinction', 'reinstatement', 'post', 'learning', 'fill', 'extinction', 'reinstatement', 'post', 'fill', 'extinction', 'reinstatement', 'switch', 'post', 'learning', 'fill', 'extinction', 'reinstatement', 'post')

data$stateName <- mapping[data$state]
```

# save simmed data #

```{r}
write.csv(data,'data/simulations/simulated_choice_data.csv')
```