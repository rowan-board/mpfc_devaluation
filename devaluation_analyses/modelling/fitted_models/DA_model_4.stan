/* RL directed action model 4 (two channel, motivation biased) : lr and tau
*  author: RB
*/
...
generated quantities{
  ...
  //real bg[N,T];
  //real chosen_net[N,T];
  //real chosen_devalued[N,T];
  //real chosen_diff[N,T];
  //real unchosen_net[N,T];
  //real unchosen_devalued[N,T];
  //real unchosen_diff[N,T];
  //real chosen_red[N,T];
  //real chosen_green[N,T];
  // real chosen_colour_diff[N,T];
    ...
    for (t in 1:T) {
      ...
      //trialwise_loglik[i,t] = -1;
       ...
       for (t in 1:(Tsubj[i])) {
         ...
         // chosen_devalued[i,t] = (m[1]*Q[1,choice[i,t]]) + (m[2]*Q[2,choice[i,t]]);
         // chosen_diff[i,t] = chosen_net[i,t] - chosen_devalued[i,t];
         // unchosen_net[i,t] = Q[1,3-choice[i,t]] + Q[2,3-choice[i,t]];
         // unchosen_devalued[i,t] = (m[1]*Q[1,3-choice[i,t]]) + (m[2]*Q[2,3-choice[i,t]]);
         //  unchosen_diff[i,t] = unchosen_net[i,t] - unchosen_devalued[i,t];
         // chosen_red[i,t] = Q[1,choice[i,t]];
         // chosen_green[i,t] = Q[2,choice[i,t]];
         // chosen_colour_diff[i,t] = chosen_red[i,t] - chosen_green[i,t];
