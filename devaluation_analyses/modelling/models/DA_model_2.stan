/* RL directed action model 2 (simple_RW): LR and Tau
*  author: RB
*/
...
data {
     ...
     matrix[N,2] blockStart;   // vector containing the trial number where the new block starts, and EVs should be reset
...
model {
     ...
     for (i in 1:N) {
             vector[Nopt] v;          // vector of expected values
             vector[Nopt] pe;         // vector of prediction errors biased by LR
             ...
             for (t in 1:(Tsubj[i])) {
             		...
             		pe[choice[i,t]] = lr[i]*(rwd[i,t] - v[choice[i,t]]);
             		
             		v[choice[i,t]] = v[choice[i,t]] + pe[choice[i,t]];
