...
ex.noBreaks         = 0; % how many breaks should we have
ex.pauseState         = 0; % in what state should we break?
ex.pauseStateTime     = 0; % on what trial within above state should we break?
ex.blockNo = 1;
...
ex.preferenceRecency   = 8; 
% What constitutes a 'preference'? at least 69% of recent choices should conform. 
ex.preferenceThreshold = 0.69; 
% reward consistency refers to the degree of outcome stochasticity
ex.rewardConsistent = 0.85;
% prefer after refers to the number of trials completed before the prefency
% count begins, typically only during reinstatement to maximse trials here
ex.preferAfter = 4;
...
ex.rewardSchedule = {   %        |    UNTIL:           |    BANK
  ...
  % state 1 removed to reduce trials
  ...
  [  H       0       0      H      0     0       4       0      3  ] % wait 8 trials (see if preference remains after banking Red)
  ...
  [  0       H       H      0      0     0       4       0      3  ] % wait 8 trials
  
  % devaluation test on green with new contingencies
  [  0       H       L      0      0     1      20       0      3  ] % press A, obtain green
...
ex.coinYdistance        = 0.15;
ex.coinSize             = 50; 
ex.coinColour           = { [220 80 80] , [90 200 90] };
ex.ovalFill             = [150 150 150];   % colour when oval is empty 
%% TIMINGS
ex.foreperiod           = 0.3;             % seconds for bank screen
% removed as jittering feedback delay with poisson distribution
...
ex.timeout              = 2.0;
ex.minJitter            = 0.5;
ex.jitters = ex.minJitter + rand(250,1) + 0.5*PoissonSamp(1,250);
ex.scannerTimeStamps = zeros(1479,1);
ex.scannerVolumes = 0;
%% GAMBLE
ex.dice              = {'A','B'};          % labels of dice
if ex.fMRI
    ex.keys = [49,52];  % button box keys, 1 and 4
    ex.scannerKey = 53;
else                                               % no.keys 1-9, 1-4=left, 5=start fMRI, 6-9=right
...
Game.break = 0   ;       % initalise number of breaks completed
...
% tr:  trial parameters (and returned result for this trial)
    ...
    tr.bank = Game.bank;
    tr.purse = Game.purse;
    tr.diceRollTime = ex.jitters(tr.allTrialIndex);
    tr.break = Game.break;
    
    %%%% added 18-7-2012 - do 'End of Block' after fixed number of trials?
    [x y b]=GetMouse; oldp=[x y];            % check cursor pos
    % end block when return to state 2, start of new run, allTrialIndex >
    % 100 to ensure we don't break at the start of run 1
    % count number of breaks so we only do 2, last block can be slightly
    % shorter or longer, doesn't make sense to have break right at end
  
    
    if tr.allTrialIndex > 100 && tr.state == ex.pauseState && tr.stateTime == ex.pauseStateTime && tr.break < ex.noBreaks 
    ...
    tr.fives = [];
    isfivedown = false; % keep track of whether scanner key up or down
    while ~tr.chosenDice                    % while no choice made yet,
      [z z kcode]=KbCheck;                  % check keyboard
      if kcode(27) tr.R=pa.R_ESCAPE; return; end 
      if kcode(pa.scannerKey)>0 && ~ isfivedown
        isfivedown = true; 
        tr.fives = [tr.fives, GetSecs()];
      end
      if kcode(pa.scannerKey) == 0 
          isfivedown = false;
      end
      ...
      if GetSecs > tr.startDice + pa.timeout
      ...
      % for button boxes, changed as only using outside buttons now
      if ex.fMRI
          ki = find(kcode(pa.keys)); % use keyboard?
      end
    ...
    if tr.chosenDice < 3
        ...
        %%%% 3. Outcome
        WaitSecs(tr.diceRollTime);                            % dice roll time......
        ...
        % flip win currency in accordance with consistent mapping 
        tr.randRwd = rand;
        if tr.randRwd > ex.rewardConsistent
           tr.winnings = flip(tr.winnings);
        end
    else % when no button is pressed
        tr.chosenWinProbs = [0,0];
        tr.unchosenDice = 0;
        tr.rand = 0;
        tr.winnings = [0,0];
        tr.randRwd = 0;
    ...
    % adaptation to wait only if you don't win or win purse is not full
    % if win and purse full then skip this delay so scene 5 can be drawn
    ...
    tr.isPurseFull = tr.purse >= pa.purseSize; % check if purse is full
    ...
    if tr.totalWin>0 % if we won
        ...
        % if you don't win anything then the win purse is not full
    else tr.isWinPurseFull = false;
    end
    
    if tr.isWinPurseFull == 1
        % do nothing, don't draw reward
    elseif tr.isWinPurseFull == 0
        drawScene(scr,pa,tr,4); % draw outcome
    end
    ...
    if tr.totalWin>0 % if we won:
      ...
      if tr.isWinPurseFull % if so
        ...
        ap=audioplayer(scr.soundData{4}, scr.soundFs{4}); 
        play(ap); % Play win sound here to keep consistency between between wins when purse full and not
    ...
    for i=1:2 % for each option
      ...
      if bitand( crit.until_prefer, i ) % do we change when this option is preferred?
        % check if purse is full, need to add trials to reinstatement pre preference check 
        if tr.isPurseFull(1)+tr.isPurseFull(2)>0
            if tr.stateTime > pa.preferenceRecency + pa.preferAfter
                if tr.recentPreference(i)
                    tr.goToNextState = true;
                end
            end
        else % same check but without additional trials
             if tr.stateTime > pa.preferenceRecency
                if tr.recentPreference(i)
                    tr.goToNextState = true;
                end
             end
    ...
    if tr.goToNextState 
      ...
      if Game.state > size(pa.rewardSchedule,1)
        Game.state =2; % wrap around if reached the end
      end
    ...
    for i=1:2 % for each currency
      if bitand( tr.moveToBank, i ) % are we moving this purse to bank?
        drawScene(scr,pa,tr,6); % redraw new purse and bank balance
        WaitSecs(0.2);
        while tr.purse(i) %
          banked = banked + 1;
...
% at the end of the trial.
    ...
    drawTextCentred(scr, sprintf('Bank: £%0.2f', tr.bank) ...
      , pa.fgColour, scr.centre+[ 0.15*scr.ssz(1) 0.1*scr.ssz(2) ]);
    drawTextCentred(scr, 'Purse:' ...
      , pa.fgColour, scr.centre+[ -0.17*scr.ssz(1) 0.1*scr.ssz(2) ]);
    for i=1:2 % for each colour currency
      for j=1:pa.purseSize(i) % for each coin slot in the purse
        if j > tr.purse(i) % if the slot is empty
          ...
          colour = pa.coinColour{i};
        end
        % calculate position
        coin_centre = scr.centre + [ (j-1)*pa.coinXdistance-343 ((i-1)*pa.coinYdistance+0.2)*scr.ssz(2)];
        % draw coin
        Screen(cmd, scr.w, colour, [coin_centre coin_centre] + [ -1 -1 2 2 ] * pa.coinSize );
        Screen('FrameOval', scr.w, pa.coinColour{i}, [coin_centre coin_centre]+ [ -1 -1 2 2 ] * pa.coinSize, [4]); 
      end % next coin
    end % next colour
    ...
    if(stage>2 && tr.chosenDice < 3) % draw chosen dice
    ...
    if(stage==4) % draw outcome
      ...
      if win_colour > 0
        ...
        Screen('FillOval',scr.w,pa.coinColour{win_colour}, [scr.centre scr.centre] + [-1 -1 2 2] * pa.coinSize -20 );
        drawTextCentred(scr, formatMoney(win_val), pa.fgColour, scr.centre);
      end
    end
    ...
    if(stage==5) % add purse full text
      ...
      drawTextCentred(scr, 'Purse is full!', pa.fgColour, scr.centre + [0,-0.1*scr.ssz(2)]);
    end
    if stage==6
      drawTextCentred(scr, 'Banked', pa.fgColour, scr.centre + [0,-0.1*scr.ssz(2)]);
    end
